...
const myVideo = document.getElementById('videoToPlay');

function videoBack() {
  const timeCurrent = myVideo.currentTime - 3;
...
window.onload = function () {
  getData((gögn) => {
    ...
    for (let i = 0; i < videoInfo.related.length; i++) {
      ...
      const videoRelatedElement = document.createElement('div');
      videoRelatedElement.setAttribute('class', 'col col-4 col-md-10');
      // býr til link á videið
      const videoRelatedClass = document.createElement('a');
      ...
      const videoRelatedImage = document.createElement('img');
      videoRelatedImage.setAttribute('src', relatedVideo.poster);
      // býr til div fyir bottom cardið
      const videoRelatedBottom = document.createElement('div');
      ...
      const videoRelatedTitle = document.createElement('h3');
      ...
      const videoDate = new Date(relatedVideo.created);
